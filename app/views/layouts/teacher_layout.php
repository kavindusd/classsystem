...
<head>
    ...
    <title><?= $title ?? 'Teacher' ?> — <?= APP_NAME ?></title>
...
<body>
    ...
    <aside class="sidebar" id="appSidebar">
        <div class="sidebar-logo flex items-center justify-between">
            <a href="<?= APP_URL ?>/teacher" class="flex items-center gap-2 group">
        ...
        $navItems = [
            ['icon' => 'fa-house',           'url' => '/teacher',               'label' => 'Overview',      'match' => '#/teacher$#'],
            ['icon' => 'fa-calendar-days',   'url' => '/teacher/schedule',      'label' => 'My Schedule',   'match' => '#/teacher/schedule#'],
            ['icon' => 'fa-book',            'url' => '/teacher/courses',       'label' => 'My Classes',    'match' => '#/teacher/courses#'],
            ['icon' => 'fa-users',           'url' => '/teacher/students',      'label' => 'My Students',   'match' => '#/teacher/students#'],
            ['icon' => 'fa-graduation-cap',  'url' => '/teacher/grading',       'label' => 'Grade Book',    'match' => '#/teacher/grading#'],
            ['icon' => 'fa-bell',            'url' => '/teacher/notifications', 'label' => 'Messages',      'match' => '#/teacher/notifications#'],
            ['icon' => 'fa-wallet',          'url' => '/teacher/earnings',      'label' => 'Earnings',      'match' => '#/teacher/earnings#'],
        ];
        ...
        <a href="<?= APP_URL ?>/teacher/settings" class="nav-item <?= strpos($curr, '/teacher/settings') !== false ? 'active' : '' ?>">
    ...
    <div class="main">
        <header class="topbar">
            <div class="flex items-center gap-3">
                ...
                <span class="topbar-title hidden sm:block"><?= htmlspecialchars($title ?? 'Teacher Portal') ?></span>
                <span class="topbar-title sm:hidden text-lg"><?= htmlspecialchars($title ?? 'Teacher') ?></span>
            </div>
            <div class="topbar-right">
                <span class="topbar-welcome hidden md:block">Teacher: <strong><?= htmlspecialchars($user['name'] ?? 'Teacher') ?></strong></span>
                <div class="topbar-avatar"><?= strtoupper(substr($user['name'] ?? 'T', 0, 2)) ?></div>
            </div>
        </header>
