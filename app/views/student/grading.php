<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Academic Grading</h1>
        <p class="text-gray-500 text-sm">Select a course to view your exams and grades.</p>
    </div>
</div>
...
<div class="pb-10">
    <?php if (empty($enrolledCourses)): ?>
        <div class="bg-white py-16 text-center rounded-xl border border-dashed border-gray-300 shadow-sm">
            ...
            <h3 class="font-bold text-gray-900 mb-1">No Courses Enrolled</h3>
            <p class="text-gray-500 text-sm">You have not enrolled in any courses yet.</p>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($enrolledCourses as $c): ?>
            <a href="<?= APP_URL ?>/student/grading/<?= $c['id'] ?>" class="block group h-full">
                <div class="bg-white border border-gray-200 p-6 rounded-xl hover:border-emerald-500 shadow-sm hover:shadow-md transition-all h-full flex flex-col">
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-emerald-600">View Grades</span>
