<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    <div>
        ...
        <p class="text-gray-500 text-sm">Your synchronized teaching schedule for the current cycle.</p>
    </div>
</div>
...
<div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
    <a href="<?= APP_URL ?>/teacher/schedule?week=<?= $weekOffset - 1 ?>" class="px-4 py-2 bg-gray-50 text-gray-600 rounded-lg font-bold text-xs hover:bg-gray-100 transition-colors flex items-center gap-2">
    ...
    <div class="flex flex-col items-center text-center">
        ...
        <?php else: ?>
            <a href="<?= APP_URL ?>/teacher/schedule" class="text-[10px] font-bold text-emerald-600 hover:underline mt-1 uppercase tracking-wider">Return to Today</a>
        <?php endif; ?>
    </div>
...
<?php if (empty($byDate)): ?>
    <div class="bg-white py-16 text-center rounded-xl border border-dashed border-gray-300">
        ...
        <p class="text-gray-400 text-sm">No teaching sessions scheduled for this period.</p>
    </div>
<?php else: ?>
                ...
                <div class="flex flex-col gap-3 h-full">
                    ...
                    <?php else: ?>
                        <?php foreach ($classesForDay as $c): ?>
                            <a href="<?= APP_URL ?>/teacher/courses/<?= $c['course_id'] ?>" class="block group h-full">
                                <div class="bg-white border border-gray-200 p-4 rounded-xl shadow-sm hover:border-emerald-500 transition-colors h-full flex flex-col relative overflow-hidden">
                                    ...
                                    <div class="mt-auto flex flex-col gap-1.5">
                                        ...
                                        <div class="text-[10px] font-bold text-gray-400">
                                            Grade <?= htmlspecialchars($c['grade'] ?? 'All') ?>
                                        </div>
                                    </div>
                                </div>
                            </a>
